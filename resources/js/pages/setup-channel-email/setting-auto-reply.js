// =====================================================
// Setting Auto Reply Page Module
// Extracted from: pages/setup-channel-email/setting-auto-reply/index.blade.php
// Reads config from: #setting-auto-reply-config data-* attrs
// =====================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.loadTable = function(page = 1) {
        const config = document.getElementById('setting-auto-reply-config');
        if (!config) return;
        ...
        .catch(err => {
            console.error(err);
            if (tableBody) tableBody.innerHTML = '<tr><td colspan="4" class="px-4 py-12 text-center text-red-400 font-bold italic">Error loading data. Please try again.</td></tr>';
        });
    };
    ...
    function renderTable(rows) {
        ...
        if (!rows || rows.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="4" class="px-4 py-12 text-center text-gray-500"><div class="flex flex-col items-center gap-2"><i class="bx bx-folder-open text-4xl opacity-20"></i><p>No auto reply settings found</p></div></td></tr>';
            return;
        }
