...
const TABLE_CONFIG = {
  ...
  const renderRow = (item) => `
    <tr class="hover:bg-blue-500/[0.03] transition-colors">
      <td class="px-6 py-4 font-mono text-blue-400 text-center">${item.id}</td>
      <td class="px-6 py-4 text-gray-100 font-medium">${window.CrudTable.escHtml(item.name)}</td>
      <td class="px-6 py-4 text-center text-gray-400">${item.start_date}</td>
      <td class="px-6 py-4 text-center text-gray-400">${item.end_date}</td>
      ...
      <td class="px-6 py-4 text-center">
        <div class="flex justify-center gap-2">
          <button onclick="openEditModal(${item.id}, '${window.CrudTable.escJs(item.name)}', '${item.start_date}', '${item.end_date}', '${window.CrudTable.escJs(item.status)}')" class="w-8 h-8 rounded-lg bg-blue-500/10 hover:bg-blue-500/20 flex items-center justify-center text-blue-400 border border-blue-500/20"><i class='bx bx-edit-alt'></i></button>
...
window.openEditModal = function(id, name, start, end, status) {
