    ...
    csrf: configElement.dataset.csrf
        ...
        const renderRow = (item) => `
            <tr class="hover:bg-blue-500/[0.03] transition-colors">
                ...
                <td class="px-6 py-4 text-gray-300">${window.CrudTable.escHtml(item.type_name)}</td>
                <td class="px-6 py-4 text-gray-300">${window.CrudTable.escHtml(item.category_name)}</td>
                ...
                <td class="px-6 py-4 text-center">
                    <div class="flex justify-center gap-2">
                        <button onclick="openEditModal(${item.id}, ${item.data_brand_name_id ?? 'null'}, ${item.data_type_id ?? 'null'}, ${item.data_category_id ?? 'null'}, '${window.CrudTable.escJs(item.name)}', '${window.CrudTable.escJs(item.status)}')" class="w-8 h-8 rounded-lg bg-blue-500/10 hover:bg-blue-500/20 flex items-center justify-center text-blue-400 border border-blue-500/20"><i class='bx bx-edit-alt'></i></button>
...
window.openCreateModal = function() {
            ...
            ['modal_brand_id','modal_type_id','modal_category_id','modal_name'].forEach(id => { document.getElementById(id).value = ''; });
            document.getElementById('modal_status').value = 'Aktif';
            window.CrudTable.openModal('masterModal');
        }

window.openEditModal = function(id, brand_id, type_id, cat_id, name, status) {
            ...
            document.getElementById('modal_category_id').value = cat_id ?? '';
