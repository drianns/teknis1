        ...
        const renderRow = (item) => `
            <tr class="hover:bg-blue-500/[0.03] transition-colors">
                ...
                <td class="px-6 py-4 text-center">
                    <span class="px-2 py-1 bg-gray-700/50 rounded text-gray-100 text-[11px] font-medium border border-gray-600/50">${window.CrudTable.escHtml(item.group_name || '-')}</span>
                </td>
                ...
                <td class="px-6 py-4 text-center">
                    <div class="flex justify-center gap-2">
                        <button onclick="openEditModal(${item.id}, ${item.data_group_name_id || 'null'}, '${window.CrudTable.escJs(item.name)}', '${window.CrudTable.escJs(item.status)}')" class="w-8 h-8 rounded-lg bg-blue-500/10 hover:bg-blue-500/20 flex items-center justify-center text-blue-400 border border-blue-500/20"><i class='bx bx-edit-alt'></i></button>
...
window.openEditModal = function(id, groupId, name, status) {
...
window.getFormData = function() {
            return {
                data_group_name_id: document.getElementById('modal_group_id').value,
