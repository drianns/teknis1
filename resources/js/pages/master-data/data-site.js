          ...
          : '<x-status-badge status="Non Aktif" />'}
                ...
                <td class="px-6 py-4 text-center">
                    <div class="flex justify-center gap-2">
                        <button onclick="openEditModal(${item.id}, '${window.CrudTable.escJs(item.name)}', '${window.CrudTable.escJs(item.location)}', '${window.CrudTable.escJs(item.status)}')" class="w-8 h-8 rounded-lg bg-blue-500/10 hover:bg-blue-500/20 flex items-center justify-center text-blue-400 border border-blue-500/20"><i class='bx bx-edit-alt'></i></button>
...
window.openEditModal = function(id, name, loc, status) {
            document.getElementById('modalTitle').innerText = 'Edit Data Site';
            document.getElementById('editId').value = id;
            document.getElementById('modal_name').value = name;
            document.getElementById('modal_location').value = loc;
