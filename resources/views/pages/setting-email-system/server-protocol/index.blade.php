...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <header class="flex-shrink-0 mb-3 px-6 pt-4">
            <h1 class="text-[28px] font-bold text-white tracking-tight mb-2">Data Email Service Protocol</h1>
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2">/</span>
                <span class="current text-blue-500 font-semibold">Data Email Service Protocol</span>
            </div>
        </header>
...
<script>
    const AJAX_URL = "{{ route('setting-email-system.server-protocol.getData') }}";
    let searchTimeout;
    ...
    function renderTable(rows) {
        ...
        if (!rows || rows.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="3" class="px-4 py-12 text-center text-gray-500"><div class="flex flex-col items-center gap-2"><i class="bx bx-folder-open text-4xl opacity-20"></i><p>No service protocols found</p></div></td></tr>';
            return;
        }
