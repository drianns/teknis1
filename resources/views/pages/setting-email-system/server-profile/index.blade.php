...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <header class="flex-shrink-0 mb-3 px-6 pt-4">
            <h1 class="text-[28px] font-bold text-white tracking-tight mb-2">Data Email Server Profile</h1>
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2">/</span>
                <span class="current text-blue-500 font-semibold">Data Email Server Profile</span>
            </div>
        </header>
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-900/50">
                        <tr>
                            ...
                            <th class="sticky top-0 z-10 bg-gray-900 px-4 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest">IP Server Name</th>
                            ...
                            <th class="sticky top-0 z-10 bg-gray-900 px-4 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest">User Login</th>
                            <th class="sticky top-0 z-10 bg-gray-900 px-4 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Password</th>
...
<script>
    ...
    function loadTable(page = 1) {
        ...
        .catch(err => {
            console.error(err);
            tableBody.innerHTML = '<tr><td colspan="7" class="px-4 py-12 text-center text-red-400 font-bold italic">Error loading data. Please try again.</td></tr>';
        });
    }
    ...
    function renderTable(rows) {
        ...
        if (!rows || rows.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="7" class="px-4 py-12 text-center text-gray-500"><div class="flex flex-col items-center gap-2"><i class="bx bx-folder-open text-4xl opacity-20"></i><p>No server profiles found</p></div></td></tr>';
            return;
        }
        ...
        tableBody.innerHTML = rows.map(row => `
            <tr class="hover:bg-blue-500/[0.03] transition-colors group/row">
                ...
                <td class="px-4 py-3 font-medium text-white whitespace-nowrap">${esc(row.name)}</td>
                <td class="px-4 py-3 text-gray-300 whitespace-nowrap">${esc(row.ip || '-')}</td>
                <td class="px-4 py-3 text-gray-300 whitespace-nowrap">${esc(row.db || '-')}</td>
                <td class="px-4 py-3 text-gray-300 whitespace-nowrap">${esc(row.user || '-')}</td>
                <td class="px-4 py-3 text-gray-400 font-mono whitespace-nowrap">${row.pass ? '••••••••' : '-'}</td>
