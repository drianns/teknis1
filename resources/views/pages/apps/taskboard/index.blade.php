            ...
            <div
                ...
                <div
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                        ...
                        <div class="flex items-center gap-2 px-3 py-1.5 bg-gray-900 border border-gray-700/50 rounded-xl">
                            ...
                            <form method="GET" action="{{ route('apps.taskboard') }}">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-700/50">
                            @forelse($resultTickets as $ticket)
                                                    <tr class="hover:bg-blue-500/[0.03] transition-colors group/row">
                                                        <td class="px-3 py-3 whitespace-nowrap">
                                                            <span class="text-sm font-medium text-gray-300">{{ $ticket->ticket_number }}</span>
                                                        </td>
                                                        <td class="px-3 py-3 whitespace-nowrap">
                                                            <span class="text-sm font-medium text-white max-w-[150px] truncate block"
                                                                title="{{ $ticket->extra_data->name ?? '-' }}">{{ $ticket->extra_data->name ?? '-' }}</span>
                                                        </td>
                                                        <td class="px-3 py-3 whitespace-nowrap">
                                                            <span class="text-xs text-gray-400">{{ $ticket->category->nama_kategori ?? '-' }}</span>
                                                        </td>
                                                        <td class="px-3 py-3 whitespace-nowrap">
                                                            <span class="text-xs text-gray-400">{{ $ticket->sla ?? '-' }}</span>
                                                        </td>
                                                        <td class="px-3 py-3 whitespace-nowrap">
                                                            <span class="text-[10px] font-bold text-gray-400 block leading-tight">
                                                                {!! str_replace(' Days ', '<br>Days ', $ticket->note_sla ?? '-') !!}
                                                            </span>
                                                        </td>
                                                        <td class="px-3 py-3 whitespace-nowrap">
                                                            <span class="text-xs text-gray-400">Layer {{ $ticket->ticket_position ?? '1' }}</span>
                                                        </td>
                                                        <td class="px-3 py-3 whitespace-nowrap">
                                                            <span class="text-xs text-gray-300">{{ $ticket->user_agent->username ?? '-' }}</span>
                                                        </td>
                                                        <td class="px-3 py-3 whitespace-nowrap">
                                                            <span class="text-xs text-gray-400">{{ $ticket->department ?? '-' }}</span>
                                                        </td>
                                                        <td class="px-3 py-3 whitespace-nowrap">
                                                            @php
                                                                $pillStyles = [
                                                                ...
                                                                $statusKey = strtolower($ticket->status);
                                                                $style = $pillStyles[$statusKey] ?? 'bg-gray-500 text-white';
                                                            @endphp
                                                            <span
                                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold {{ $style }} tracking-wide">
                                                        ...
                                                        <td class="px-3 py-3 whitespace-nowrap">
                                                            <div class="flex flex-col">
                                                                <span
                                                                    class="text-xs font-bold text-gray-300">{{ $ticket->created_at->format('n/j/Y') }}</span>
                                                                <span
                                                                    class="text-[10px] font-medium text-gray-500">{{ $ticket->created_at->format('g:i:s A') }}</span>
                                                            </div>
                                                        </td>
                                    ...
                                    'agent' => $ticket->user_agent->username ?? '',
                                    'posisi' => 'Layer ' . ($ticket->ticket_position ?? '1'),
                ...
                <div
                    ...
                    <div class="text-[11px] font-bold text-gray-500 uppercase tracking-widest">
                        Showing <span class="text-white">{{ $resultTickets->firstItem() ?? 0 }}</span> - <span
                            class="text-white">{{ $resultTickets->lastItem() ?? 0 }}</span> of <span
                            class="text-blue-400">{{ $resultTickets->total() }}</span> tickets
                    </div>
                    <div>
                        @if($resultTickets->hasPages())
                            {{ $resultTickets->appends(request()->query())->links('pages.taskboard.pagination') }}
                        @else
                            {{-- Static Pagination for Empty State (Custom Dark Mode Style to ensure consistency) --}}
