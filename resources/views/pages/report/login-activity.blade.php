...
@section('content')
<div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900 report-login-activity-page">

    {{-- Page Header --}}
    <header class="flex-shrink-0 mb-3 px-6 pt-4">
        <div class="flex justify-between items-center mb-2">
            <h1 class="text-[28px] font-bold text-white tracking-tight">Data Login Activity</h1>
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2 text-gray-600">/</span>
                <span class="current text-blue-500 font-semibold">Login Activity</span>
            </div>
        </div>
    </header>
    ...
    <div class="flex-1 flex flex-col p-4 lg:p-6 lg:pt-0 pt-0 overflow-hidden w-full gap-4">
        ...
        <div class="bg-gray-800/80 backdrop-blur-md rounded-2xl border border-gray-700/50 shadow-xl ring-1 ring-white/5 shrink-0 px-5 py-4">
            <form id="filter-form" method="GET" action="{{ route('report.login-activity') }}">
                <div class="flex flex-wrap items-end gap-4">
                    ...
                    <a href="{{ route('report.login-activity') }}"
        ...
        <div class="table-container bg-gray-800/80 backdrop-blur-md rounded-2xl border border-gray-700/50 shadow-2xl ring-1 ring-white/5 flex-1 flex flex-col min-h-0 overflow-hidden">
            ...
            <div class="table-wrapper flex-1 overflow-auto w-full custom-scrollbar">
                <table class="data-table w-full text-left border-collapse table-fixed min-w-[700px]"
                       x-data="loginTableData()">
                    <thead class="bg-gray-900/50 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                        <tr>
                            <th draggable="true" @dragstart="handleDragStart($event,'ID')" @dragend="handleDragEnd($event)"
                                class="sticky top-0 z-10 bg-gray-900 draggable-header px-3 py-3 w-20 hover:bg-gray-800/80 transition-colors group cursor-grab active:cursor-grabbing border-r border-gray-700/40 whitespace-nowrap">
                                <div class="flex items-center justify-between"><span>ID</span>
                            ...
                            <th draggable="true" @dragstart="handleDragStart($event,'Description')" @dragend="handleDragEnd($event)"
                                ...
                                <div class="flex items-center justify-between"><span>Description</span>
                                <i class='bx bx-grid-vertical text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity'></i></div>
                            </th>
                            <th draggable="true" @dragstart="handleDragStart($event,'Date')" @dragend="handleDragEnd($event)"
                                class="sticky top-0 z-10 bg-gray-900 draggable-header px-3 py-3 hover:bg-gray-800/80 transition-colors group cursor-grab active:cursor-grabbing whitespace-nowrap">
                                <div class="flex items-center justify-between"><span>Date</span>
                                <i class='bx bx-grid-vertical text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity'></i></div>
                            </th>
                        </tr>
                        ...
                        <tr class="bg-gray-900/80">
                            ...
                            <td class="px-2 py-1.5 border-r border-gray-700/40">
                                <input type="text" id="filter-agent" placeholder=""
                    ...
                    <tbody id="login-tbody" class="divide-y divide-gray-700/50 text-sm text-gray-300">
                        @forelse($loginData as $index => $row)
                            @php
                                $descColor = match(strtolower($row['description'])) {
                                    'login'  => 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30',
                                    'logout' => 'bg-rose-500/20 text-rose-400 border-rose-500/30',
                            ...
                            @endphp
                            <tr class="hover:bg-blue-500/[0.03] transition-colors login-row"
                                data-agent="{{ strtolower($row['agent']) }}"
                                data-desc="{{ strtolower($row['description']) }}">
                                <td class="px-3 py-3 whitespace-nowrap font-mono text-gray-400 text-xs">
                                    {{ $row['id'] }}
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-gray-300 text-xs font-medium">{{ $row['agent'] }}</span>
                                    </div>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                            ...
                            <tr>
                                <td colspan="4" class="py-16 text-center">
                                    <div class="flex flex-col items-center gap-3 text-gray-600">
                                        ...
                                        <p class="text-sm font-medium">No login activity records found</p>
                                        @if(request('start_date') || request('end_date'))
            ...
            <div class="px-5 py-3 border-t border-gray-700/50 flex flex-wrap justify-between items-center gap-4 bg-gray-800/30 shrink-0">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-4">
                    <div class="text-sm text-gray-500 hidden lg:block">
                        Showing <span class="text-white font-semibold">{{ count($loginData) }}</span> records
                    </div>
                </div>
...
<script>
    ...
    function filterTable() {
        const agent = document.getElementById('filter-agent').value.toLowerCase();
    ...
    function exportReport(format) {
        ...
        window.location.href = '{{ route("report.login-activity") }}?' + params.toString();
    }
</script>
@endsection
