...
@section('content')
<div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900 report-base-on-staff-page">

    {{-- Page Header --}}
    <header class="flex-shrink-0 mb-3 px-6 pt-4">
        <div class="flex justify-between items-center mb-2">
            <h1 class="text-[28px] font-bold text-white tracking-tight">Report Base on Staff</h1>
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2 text-gray-600">/</span>
                <span class="current text-blue-500 font-semibold">Base on Staff</span>
            </div>
        </div>
    </header>
    ...
    <div class="flex-1 flex flex-col p-4 lg:p-6 lg:pt-0 pt-0 overflow-hidden w-full gap-4">
        ...
        <div class="bg-gray-800/80 backdrop-blur-md rounded-2xl border border-gray-700/50 shadow-xl ring-1 ring-white/5 shrink-0 px-5 py-4">
            <form id="filter-form" method="GET" action="{{ route('report.base-on-staff') }}">
                <div class="flex flex-wrap items-end gap-4">
                    ...
                    <a href="{{ route('report.base-on-staff') }}"
        ...
        <div class="table-container bg-gray-800/80 backdrop-blur-md rounded-2xl border border-gray-700/50 shadow-2xl ring-1 ring-white/5 flex-1 flex flex-col min-h-0 overflow-hidden">
            ...
            <div class="table-wrapper flex-1 overflow-auto w-full custom-scrollbar">
                <table class="data-table w-full text-left border-collapse table-fixed min-w-[1350px]"
                    ...
                    <thead class="bg-gray-900/50 sticky top-0 z-20 shadow-sm text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                        <tr>
                            ...
                            <th draggable="true" @dragstart="handleDragStart($event,'Agent Name')" @dragend="handleDragEnd($event)"
                                class="bg-gray-900 draggable-header px-3 py-3 w-48 hover:bg-gray-800/80 transition-colors group cursor-grab active:cursor-grabbing border-r border-gray-700/40 whitespace-nowrap">
                                <div class="flex items-center justify-between"><span>Agent Name</span>
                                <i class='bx bx-grid-vertical text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity'></i></div>
                            </th>
                            ...
                            <th draggable="true" @dragstart="handleDragStart($event,'Assigned At')" @dragend="handleDragEnd($event)"
                                ...
                                <div class="flex items-center justify-between"><span>Assigned At</span>
                                <i class='bx bx-grid-vertical text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity'></i></div>
                            </th>
                            <th draggable="true" @dragstart="handleDragStart($event,'Handle Time')" @dragend="handleDragEnd($event)"
                                ...
                                <div class="flex items-center justify-between"><span>Handle Time</span>
                                <i class='bx bx-grid-vertical text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity'></i></div>
                            </th>
                            <th draggable="true" @dragstart="handleDragStart($event,'Escalated')" @dragend="handleDragEnd($event)"
                                class="bg-gray-900 draggable-header px-3 py-3 w-28 hover:bg-gray-800/80 transition-colors group cursor-grab active:cursor-grabbing whitespace-nowrap">
                                <div class="flex items-center justify-between"><span>Escalated</span>
                                <i class='bx bx-grid-vertical text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity'></i></div>
                            </th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-700/50 text-sm text-gray-300">
                        @forelse($tickets as $index => $ticket)
                            @php
                                ...
                                // Handle time: created → updated
                                $handleMin = $ticket->created_at
                                    ? (int) $ticket->created_at->diffInMinutes($ticket->updated_at ?? now())
                                    : 0;
                                $handleH = floor($handleMin / 60);
                                $handleM = $handleMin % 60;
                                $handleStr = sprintf('%dh %02dm', $handleH, $handleM);
                                ...
                                $isEscalated = $ticket->need_escalated;
                            @endphp
                            <tr class="hover:bg-blue-500/[0.03] transition-colors group/row">
                                ...
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <div class="flex items-center gap-2.5">
                                        <div class="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-600/50 to-purple-600/50 border border-indigo-500/40 flex items-center justify-center shrink-0 text-xs font-bold text-white">
                                            {{ strtoupper(substr($agentName, 0, 1)) }}
                                        </div>
                                        <div>
                                            <p class="text-white font-medium text-sm truncate max-w-[130px]" title="{{ $agentName }}">
                                                {{ $agentName }}
                                            </p>
                                            @if($ticket->userAgent)
                                                <p class="text-gray-600 text-[10px]">{{ $ticket->userAgent->layer ?? 'agent' }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <span class="font-mono text-xs text-gray-400">{{ $handleStr }}</span>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    @if($isEscalated)
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-red-500/20 text-red-400 border border-red-500/30">
                                            <i class='bx bx-up-arrow-alt text-xs'></i> Yes
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-gray-700/40 text-gray-500 border border-gray-600/30">
                                            No
                                        </span>
                                    @endif
                                </td>
                            ...
                            <tr>
                                <td colspan="10" class="py-16 text-center">
                                    <div class="flex flex-col items-center gap-3 text-gray-600">
                                        ...
                                        <p class="text-sm font-medium">No staff records found</p>
                                        @if(request('start_date') || request('end_date'))
    ...
    <div id="shared-report-config" class="hidden" data-endpoint="{{ route('report.base-on-staff') }}"></div>
@push('scripts')
