...
<div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900 report-thread-transaction-page">
    ...
    <header class="flex-shrink-0 mb-3 px-6 pt-4">
        <div class="flex justify-between items-center mb-2">
            <h1 class="text-[28px] font-bold text-white tracking-tight">Report Thread Transaction</h1>
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2 text-gray-600">/</span>
                <span class="current text-blue-500 font-semibold">Thread Transaction</span>
            </div>
        </div>
    </header>
    ...
    <div class="flex-1 flex flex-col p-4 lg:p-6 lg:pt-0 pt-0 overflow-hidden w-full gap-4">
        ...
        <div class="bg-gray-800/80 backdrop-blur-md rounded-2xl border border-gray-700/50 shadow-xl ring-1 ring-white/5 shrink-0 px-5 py-4">
            <form id="filter-form" method="GET" action="{{ route('report.thread-transaction') }}">
                <div class="flex flex-wrap items-end gap-4">
                    ...
                    <a href="{{ route('report.thread-transaction') }}"
                ...
                <table class="data-table w-full text-left border-collapse table-fixed min-w-[1200px]"
                    ...
                    <thead class="bg-gray-900/50 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                        <tr>
                            ...
                            <th draggable="true" @dragstart="handleDragStart($event,'Thread ID')" @dragend="handleDragEnd($event)"
                                ...
                                <div class="flex items-center justify-between"><span>Thread ID</span>
                                <i class='bx bx-grid-vertical text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity'></i></div>
                            </th>
                            ...
                            <th draggable="true" @dragstart="handleDragStart($event,'Subject')" @dragend="handleDragEnd($event)"
                                class="sticky top-0 z-10 bg-gray-900 draggable-header px-3 py-3 w-52 hover:bg-gray-800/80 transition-colors group cursor-grab active:cursor-grabbing border-r border-gray-700/40 whitespace-nowrap">
                            ...
                            <th draggable="true" @dragstart="handleDragStart($event,'Thread Status')" @dragend="handleDragEnd($event)"
                                ...
                                <div class="flex items-center justify-between"><span>Thread Status</span>
                                <i class='bx bx-grid-vertical text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity'></i></div>
                            </th>
                            <th draggable="true" @dragstart="handleDragStart($event,'Ticket Status')" @dragend="handleDragEnd($event)"
                                class="sticky top-0 z-10 bg-gray-900 draggable-header px-3 py-3 w-32 hover:bg-gray-800/80 transition-colors group cursor-grab active:cursor-grabbing border-r border-gray-700/40 whitespace-nowrap">
                                <div class="flex items-center justify-between"><span>Ticket Status</span>
                                <i class='bx bx-grid-vertical text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity'></i></div>
                            </th>
                    ...
                    <tbody class="divide-y divide-gray-700/50 text-sm text-gray-300">
                        @forelse($threads as $index => $thread)
                            @php
                                ...
                                // Channel icon
                                $channelName = $thread->channel->name ?? ('CH-' . $thread->channel_id);
                                $channelType = strtolower($thread->channel->type ?? '');
                                ...
                                $channelColor = match(true) {
                                    str_contains($channelType, 'email')     => 'text-blue-400',
                                    str_contains($channelType, 'whatsapp')  => 'text-emerald-400',
                                    str_contains($channelType, 'instagram') => 'text-pink-400',
                                    str_contains($channelType, 'facebook')  => 'text-indigo-400',
                                    default                                 => 'text-gray-400',
                                };

                                // Thread status badge
                                $threadStatus = strtolower($thread->status ?? '');
                                $threadStatusClass = match(true) {
                                    str_contains($threadStatus, 'open')  => 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30',
                                    str_contains($threadStatus, 'close') => 'bg-gray-500/20 text-gray-400 border-gray-500/30',
                                    default                              => 'bg-blue-500/20 text-blue-400 border-blue-500/30',
                                };

                                // Ticket status badge
                                $ticketStatus = strtolower($ticket->status ?? '');
                                $ticketStatusClass = match(true) {
                            ...
                            <tr class="hover:bg-blue-500/[0.03] transition-colors">
                                <td class="px-3 py-3 whitespace-nowrap font-mono text-gray-500 text-xs">
                                    {{ $threads->firstItem() + $index }}
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <span class="font-mono text-xs text-gray-400 bg-gray-700/40 px-2 py-1 rounded-md border border-gray-600/30">
                                        #{{ $thread->id }}
                                    </span>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <div class="flex items-center gap-1.5">
                                        <i class='bx {{ $channelIcon }} {{ $channelColor }} text-base'></i>
                                ...
                                <td class="px-3 py-3 max-w-[180px] truncate">
                                    <span class="text-white font-medium text-sm" title="{{ $ticket->subject ?? '' }}">
                                        {{ $ticket->subject ?? '—' }}
                                    </span>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-gray-300 text-xs truncate max-w-[110px]" title="{{ $agentName }}">
                                            {{ $agentName }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border {{ $threadStatusClass }}">
                                        <span class="w-1.5 h-1.5 rounded-full {{ str_contains($threadStatus, 'open') ? 'bg-emerald-400 animate-pulse' : 'bg-current' }}"></span>
                                        {{ ucfirst($thread->status ?? 'Unknown') }}
                                    </span>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    @if($ticket)
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border {{ $ticketStatusClass }}">
                                    ...
                                    @else
                                        <span class="text-gray-600 text-xs">No ticket</span>
                                    @endif
                                </td>
                            ...
                            <tr>
                                <td colspan="9" class="py-16 text-center">
                                    <div class="flex flex-col items-center gap-3 text-gray-600">
                                        <i class='bx bx-conversation text-5xl'></i>
                                        <p class="text-sm font-medium">No thread transaction records found</p>
                                        @if(request('start_date') || request('end_date'))
            ...
            <div class="px-5 py-3 border-t border-gray-700/50 flex flex-wrap justify-between items-center gap-4 bg-gray-800/30 shrink-0">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-4">
                    <div class="text-sm text-gray-500 hidden lg:block">
                        Showing <span class="text-white font-semibold">{{ $threads->firstItem() ?? 0 }}</span>
                        to <span class="text-white font-semibold">{{ $threads->lastItem() ?? 0 }}</span>
                        of <span class="text-white font-semibold">{{ $threads->total() }}</span> results
                    </div>
                    <div>
                        {{ $threads->appends(request()->query())->links('components.pagination') }}
                    </div>
                </div>
...
<script>
    ...
    function exportReport(format) {
        ...
        window.location.href = '{{ route("report.thread-transaction") }}?' + params.toString();
    }
</script>
@endsection
