...
@section('content')
<div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900 report-interaction-ticket-page">

    {{-- Page Header --}}
    <header class="flex-shrink-0 mb-3 px-6 pt-4">
        <div class="flex justify-between items-center mb-2">
            <h1 class="text-[28px] font-bold text-white tracking-tight">Report Interaction Ticket</h1>
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2 text-gray-600">/</span>
                <span class="current text-blue-500 font-semibold">Interaction Ticket</span>
            </div>
        </div>
    </header>
    ...
    <div class="flex-1 flex flex-col p-4 lg:p-6 lg:pt-0 pt-0 overflow-hidden w-full gap-4">
        ...
        <div class="bg-gray-800/80 backdrop-blur-md rounded-2xl border border-gray-700/50 shadow-xl ring-1 ring-white/5 shrink-0 px-5 py-4">
            <form id="filter-form" method="GET" action="{{ route('report.interaction-ticket') }}">
                <div class="flex flex-wrap items-end gap-4">
                    ...
                    <a href="{{ route('report.interaction-ticket') }}"
        ...
        <div class="bg-gray-800/80 rounded-xl border border-gray-700/50 px-4 py-3 shrink-0 shadow-sm"
             x-data="dragDropData()">
            <div class="drop-zone border border-dashed border-gray-600 rounded-lg px-4 py-2.5 flex items-center min-h-[44px] transition-all duration-300"
        ...
        <div class="table-container bg-gray-800/80 backdrop-blur-md rounded-2xl border border-gray-700/50 shadow-2xl ring-1 ring-white/5 flex-1 flex flex-col min-h-0 overflow-hidden">
                ...
                <table class="data-table w-full text-left border-collapse table-fixed min-w-[1300px]"
                    ...
                    <thead class="bg-gray-900/50 sticky top-0 z-20 shadow-sm text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                        <tr>
                            ...
                            <th draggable="true" @dragstart="handleDragStart($event,'Interaction Type')" @dragend="handleDragEnd($event)"
                                ...
                                <div class="flex items-center justify-between"><span>Interaction Type</span>
                                <i class='bx bx-grid-vertical text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity'></i></div>
                            </th>
                            <th draggable="true" @dragstart="handleDragStart($event,'Channel')" @dragend="handleDragEnd($event)"
                                class="bg-gray-900 draggable-header px-3 py-3 w-36 hover:bg-gray-800/80 transition-colors group cursor-grab active:cursor-grabbing border-r border-gray-700/40 whitespace-nowrap">
                                <div class="flex items-center justify-between"><span>Channel</span>
                                <i class='bx bx-grid-vertical text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity'></i></div>
                            </th>
                            ...
                            <th draggable="true" @dragstart="handleDragStart($event,'Status')" @dragend="handleDragEnd($event)"
                                class="bg-gray-900 draggable-header px-3 py-3 w-28 hover:bg-gray-800/80 transition-colors group cursor-grab active:cursor-grabbing border-r border-gray-700/40 whitespace-nowrap">
                                <div class="flex items-center justify-between"><span>Status</span>
                                <i class='bx bx-grid-vertical text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity'></i></div>
                            </th>
                            <th draggable="true" @dragstart="handleDragStart($event,'Merged')" @dragend="handleDragEnd($event)"
                                class="bg-gray-900 draggable-header px-3 py-3 w-24 hover:bg-gray-800/80 transition-colors group cursor-grab active:cursor-grabbing border-r border-gray-700/40 whitespace-nowrap">
                                <div class="flex items-center justify-between"><span>Merged</span>
                                <i class='bx bx-grid-vertical text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity'></i></div>
                            </th>
                            <th draggable="true" @dragstart="handleDragStart($event,'Created At')" @dragend="handleDragEnd($event)"
                                class="bg-gray-900 draggable-header px-3 py-3 w-40 hover:bg-gray-800/80 transition-colors group cursor-grab active:cursor-grabbing whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-gray-700/50 text-sm text-gray-300">
                        @forelse($tickets as $index => $ticket)
                            @php
                                $agentName   = $ticket->user_agent->full_name
                                    ?? ($ticket->user_agent->user->name ?? 'Unassigned');
                                $channelName = $ticket->channel->name ?? ('CH-' . $ticket->channel_id);
                                $channelType = strtolower($ticket->channel->type ?? '');

                                // Interaction type badge
                                $flagLabel = $ticket->flaging_label;
                                $flagClass = match($ticket->flaging) {
                                    1 => 'bg-green-500/20 text-green-400 border-green-500/30',   // Inbound
                                    2 => 'bg-amber-500/20 text-amber-400 border-amber-500/30',   // Outbound
                                    3 => 'bg-purple-500/20 text-purple-400 border-purple-500/30', // Chat
                                    4 => 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30',      // Email
                                    5 => 'bg-pink-500/20 text-pink-400 border-pink-500/30',      // Blast
                                    default => 'bg-gray-500/20 text-gray-400 border-gray-500/30',
                                };
                                $flagIcon = match($ticket->flaging) {
                                    1 => 'bx-phone-incoming',
                                    2 => 'bx-phone-outgoing',
                                    3 => 'bx-chat',
                                    4 => 'bx-envelope',
                                ...
                                $statusClass = match(true) {
                                    str_contains($status, 'open')    => 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30',
                                    str_contains($status, 'close')   => 'bg-gray-500/20 text-gray-400 border-gray-500/30',
                                    str_contains($status, 'pending') => 'bg-amber-500/20 text-amber-400 border-amber-500/30',
                                    default                          => 'bg-blue-500/20 text-blue-400 border-blue-500/30',
                                };

                                // Channel icon
                                $channelIcon = match(true) {
                                    str_contains($channelType, 'email')     => 'bx-envelope',
                                    str_contains($channelType, 'whatsapp')  => 'bxl-whatsapp',
                                    str_contains($channelType, 'instagram') => 'bxl-instagram',
                                    str_contains($channelType, 'facebook')  => 'bxl-facebook',
                                    str_contains($channelType, 'chat')      => 'bx-chat',
                                    default                                 => 'bx-globe',
                                };
                            @endphp
                            <tr class="hover:bg-blue-500/[0.03] transition-colors">
                                ...
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border {{ $flagClass }}">
                                        <i class='bx {{ $flagIcon }} text-sm'></i>
                                        {{ $flagLabel }}
                                    </span>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <div class="flex items-center gap-1.5">
                                        <i class='bx {{ $channelIcon }} text-gray-400 text-base'></i>
                                        <span class="text-gray-400 text-xs truncate max-w-[100px]">{{ $channelName }}</span>
                                    </div>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-gray-300 text-xs truncate max-w-[115px]" title="{{ $agentName }}">
                                            {{ $agentName }}
                                        </span>
                                    </div>
                                </td>
                                ...
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border {{ $statusClass }}">
                                        <span class="w-1.5 h-1.5 rounded-full {{ str_contains($status, 'open') ? 'bg-emerald-400 animate-pulse' : 'bg-current' }}"></span>
                                        {{ ucfirst($ticket->status ?? 'Unknown') }}
                                    </span>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    @if($ticket->is_merged)
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-orange-500/20 text-orange-400 border border-orange-500/30">
                                            <i class='bx bx-git-merge text-sm'></i> Yes
                                        </span>
                                    @else
                            ...
                            <tr>
                                <td colspan="10" class="py-16 text-center">
                                    <div class="flex flex-col items-center gap-3 text-gray-600">
                                        <i class='bx bx-message-square-x text-5xl'></i>
                                        <p class="text-sm font-medium">No interaction ticket records found</p>
                                        @if(request('start_date') || request('end_date'))
            ...
            <div class="px-5 py-3 border-t border-gray-700/50 flex flex-wrap justify-between items-center gap-4 bg-gray-800/30 shrink-0">
                <div class="flex flex-col sm:flex-row items-center gap-4">
                    ...
                    <div>{{ $tickets->appends(request()->query())->links('components.pagination') }}</div>
                </div>
    ...
    <div id="shared-report-config" class="hidden" data-endpoint="{{ route('report.interaction-ticket') }}"></div>
@push('scripts')
