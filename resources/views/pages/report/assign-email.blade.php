...
@section('content')
<div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900 report-assign-email-page">

    {{-- Page Header --}}
    <header class="flex-shrink-0 mb-3 px-6 pt-4">
        <div class="flex justify-between items-center mb-2">
            <h1 class="text-[28px] font-bold text-white tracking-tight">Report Assign Email</h1>
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2 text-gray-600">/</span>
                <span class="current text-blue-500 font-semibold">Assign Email</span>
            </div>
        </div>
    </header>
    ...
    <div class="flex-1 flex flex-col p-4 lg:p-6 lg:pt-0 pt-0 overflow-hidden w-full gap-4">
        ...
        <div class="bg-gray-800/80 backdrop-blur-md rounded-2xl border border-gray-700/50 shadow-xl ring-1 ring-white/5 shrink-0 px-5 py-4">
            <form id="filter-form" method="GET" action="{{ route('report.assign-email') }}">
                <div class="flex flex-wrap items-end gap-4">
                    ...
                    <a
                        href="{{ route('report.assign-email') }}"
        ...
        <div class="bg-gray-800/80 rounded-xl border border-gray-700/50 px-4 py-3 shrink-0 shadow-sm"
            ...
            <div id="drop-zone"
                ...
                {{-- Empty State --}}
                <div class="flex items-center gap-3 text-gray-500 text-sm transition-opacity duration-300"
                    ...
                    <template x-for="(column, index) in groupedColumns" :key="column">
                        <div class="flex items-center gap-1.5 px-2.5 py-1 bg-gray-900 border border-gray-600 text-gray-300 rounded-md text-xs font-medium shadow-sm animate-fade-in-up">
                            ...
                            <button @click="removeGroupedColumn(index)"
                                    class="hover:text-red-400 transition-colors ml-1">
                            ...
                            <tr class="hover:bg-blue-500/[0.03] transition-colors group/row">
                                ...
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-gray-300 truncate max-w-[130px]" title="{{ $ticket->userAgent->full_name ?? $ticket->userAgent->user->name ?? 'Unassigned' }}">
                                            {{ $ticket->userAgent->full_name ?? ($ticket->userAgent->user->name ?? 'Unassigned') }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap truncate text-gray-400">
                                    {{ $ticket->category ?? '—' }}
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    @php
                                        ...
                                        $statusClass = match(true) {
                                            ...
                                            str_contains($status, 'escalat') => 'bg-red-500/20 text-red-400 border-red-500/30',
                                ...
                                <td class="px-3 py-3 whitespace-nowrap text-xs">
                                    @php
                                        $responseMinutes = $ticket->created_at ? now()->diffInMinutes($ticket->created_at, false) : null;
                                        $responseMinutes = abs($responseMinutes ?? 0);
                                        $hours = floor($responseMinutes / 60);
                                        $mins  = $responseMinutes % 60;
                                    @endphp
                                    <span class="text-gray-400 font-mono">
                                        {{ $ticket->created_at ? sprintf('%dh %02dm', $hours, $mins) : '—' }}
                                    </span>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="py-16 text-center">
                                    <div class="flex flex-col items-center gap-3 text-gray-600">
                                        <i class='bx bx-envelope text-5xl'></i>
                                        <p class="text-sm font-medium">No assign email records found</p>
                                        @if(request('start_date') || request('end_date'))
