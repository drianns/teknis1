...
@section('content')
<div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900 report-base-on-sla-page">

    {{-- Page Header --}}
    <header class="flex-shrink-0 mb-3 px-6 pt-4">
        <div class="flex justify-between items-center mb-2">
            <h1 class="text-[28px] font-bold text-white tracking-tight">Report Base on SLA</h1>
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2 text-gray-600">/</span>
                <span class="current text-blue-500 font-semibold">Base on SLA</span>
            </div>
        </div>
    </header>
    ...
    <div class="flex-1 flex flex-col p-4 lg:p-6 lg:pt-0 pt-0 overflow-hidden w-full gap-4">
        ...
        <div class="bg-gray-800/80 backdrop-blur-md rounded-2xl border border-gray-700/50 shadow-xl ring-1 ring-white/5 shrink-0 px-5 py-4">
            <form id="filter-form" method="GET" action="{{ route('report.base-on-sla') }}">
        ...
        <div class="bg-gray-800/80 rounded-xl border border-gray-700/50 px-4 py-3 shrink-0 shadow-sm"
             x-data="dragDropData()">
            <div id="drop-zone"
                 class="drop-zone border border-dashed border-gray-600 rounded-lg px-4 py-2.5 flex items-center min-h-[44px] transition-all duration-300 relative"
                ...
                <table class="data-table w-full text-left border-collapse table-fixed min-w-[1200px]"
                    ...
                    <thead class="bg-gray-900/50 sticky top-0 z-20 shadow-sm text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                        <tr>
                            ...
                            <th draggable="true" @dragstart="handleDragStart($event,'SLA Target')" @dragend="handleDragEnd($event)"
                                class="bg-gray-900 draggable-header px-3 py-3 w-28 hover:bg-gray-800/80 transition-colors group cursor-grab active:cursor-grabbing border-r border-gray-700/40 whitespace-nowrap">
                                <div class="flex items-center justify-between">
                            ...
                            <th draggable="true" @dragstart="handleDragStart($event,'SLA Status')" @dragend="handleDragEnd($event)"
                                class="bg-gray-900 draggable-header px-3 py-3 w-32 hover:bg-gray-800/80 transition-colors group cursor-grab active:cursor-grabbing whitespace-nowrap">
                                <div class="flex items-center justify-between">
                    ...
                    <tbody class="divide-y divide-gray-700/50 text-sm text-gray-300">
                        @forelse($tickets as $index => $ticket)
                            @php
                                // SLA target: 24 hours (1440 minutes) — adjustable
                                $slaTargetMinutes = 1440;
                                $createdAt  = $ticket->created_at;
                                $updatedAt  = $ticket->updated_at;
                                $elapsedMin = $createdAt ? (int) $createdAt->diffInMinutes($updatedAt ?? now()) : 0;
                                $isMet      = $elapsedMin <= $slaTargetMinutes;

                                // Format elapsed
                                $elHours = floor($elapsedMin / 60);
                                $elMins  = $elapsedMin % 60;
                                $elapsedFormatted = sprintf('%dh %02dm', $elHours, $elMins);
                            ...
                            <tr class="hover:bg-blue-500/[0.03] transition-colors group/row">
                                ...
                                <td class="px-3 py-3 max-w-[190px] truncate">
                                    <span class="text-white font-medium" title="{{ $ticket->subject }}">
                                        {{ $ticket->subject ?? '—' }}
                                    </span>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        <div class="w-7 h-7 rounded-full bg-indigo-600/30 border border-indigo-500/30 flex items-center justify-center shrink-0">
                                            <i class='bx bx-user text-indigo-400 text-xs'></i>
                                        </div>
                                        <span class="text-gray-300 truncate max-w-[120px]" title="{{ $agentName }}">
                                            {{ $agentName }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap truncate text-gray-400 text-xs">
                                    {{ $ticket->category ?? '—' }}
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                ...
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <span class="text-gray-400 font-mono text-xs">24h 00m</span>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <span class="font-mono text-xs {{ $isMet ? 'text-emerald-400' : 'text-red-400' }}">
                                        {{ $elapsedFormatted }}
                                    </span>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    @if($isMet)
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
                                            <i class='bx bx-check text-sm'></i> Met
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-red-500/20 text-red-400 border border-red-500/30">
                                            <i class='bx bx-x text-sm'></i> Breached
                                        </span>
                                    @endif
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td colspan="9" class="py-16 text-center">
                                    <div class="flex flex-col items-center gap-3 text-gray-600">
                                        ...
                                        <p class="text-sm font-medium">No SLA records found</p>
                                        @if(request('start_date') || request('end_date'))
    ...
    <div id="shared-report-config" class="hidden" data-endpoint="{{ route('report.base-on-sla') }}"></div>
@push('scripts')
