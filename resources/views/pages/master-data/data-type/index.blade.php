...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <x-page-header title="Data Type" :breadcrumbs="['Master Data', 'Data Type']">
    ...
    <x-modal id="masterModal" title="Add New Data Type" maxWidth="max-w-lg">
    ...
    <div id="master-data-config" class="hidden"
        data-ajax-url="{{ url('data-type/data') }}"
        data-store-url="{{ route('data-type.store') }}"
        data-update-base="{{ url('data-type') }}"
        data-delete-base="{{ url('data-type') }}"
        data-csrf="{{ csrf_token() }}"></div>

    @push('scripts')
        @vite('resources/js/pages/master-data/data-type.js')
    @endpush
@endsection
