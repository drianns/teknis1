...
<div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
  <x-page-header title="Data Aux Reason" :breadcrumbs="['Master Data', 'Data Aux Reason']">
    <button onclick="openCreateModal()" class="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl transition-all shadow-lg shadow-blue-600/20 font-semibold text-sm">
      <i class='bx bx-plus-circle text-lg'></i><span>Add Aux Reason</span>
    </button>
  </x-page-header>
    <x-data-table>
      <x-slot name="thead">
...
<x-modal id="masterModal" title="Add Aux Reason" maxWidth="max-w-lg">
  ...
  <x-slot name="footer">
    ...
    <div id="master-data-config" class="hidden"
        data-ajax-url="{{ url('data-aux-reason/data') }}"
        data-store-url="{{ route('data-aux-reason.store') }}"
        data-update-base="{{ url('data-aux-reason') }}"
        data-delete-base="{{ url('data-aux-reason') }}"
        data-csrf="{{ csrf_token() }}"></div>

    @push('scripts')
        @vite('resources/js/pages/master-data/data-aux-reason.js')
    @endpush
</script>
@endsection
