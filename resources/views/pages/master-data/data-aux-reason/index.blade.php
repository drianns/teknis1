...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <header class="flex-shrink-0 mb-3 px-6 pt-4">
            <div class="flex justify-between items-center mb-2">
                <h1 class="text-[28px] font-bold text-white tracking-tight">Data Aux Reason</h1>
            ...
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2 text-gray-600">/</span>
                <span class="current text-blue-500 font-semibold">Data Aux Reason</span>
            </div>
        </header>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-900/50">
                                ...
                                <th class="sticky top-0 z-10 bg-gray-900 px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Aux Reason Name</th>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="relative flex justify-center" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.outside="open = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="absolute right-0 top-full mt-2 w-32 bg-gray-800 border border-gray-700 rounded-xl shadow-xl z-20 overflow-hidden ring-1 ring-white/5" style="display:none;">
                                            ...
                                            <form action="{{ route('data-aux-reason.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Silahkan konfirmasi untuk menghapus data ini?')">
                                                @csrf
                                                @method('DELETE')
        ...
        <div class="flex items-center justify-center min-h-screen px-4 py-12">
            ...
            <div class="relative bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-700 ring-1 ring-white/5">
                <div class="px-6 py-4 border-b border-gray-700 flex justify-between items-center bg-gray-900/50">
                    <h3 id="modalTitle" class="text-xl font-bold text-white tracking-tight">Add New Data Aux Reason</h3>
                ...
                <form id="modalForm" method="POST">
                    ...
                    <div class="p-6 space-y-5">
                        <div class="space-y-2">
                            <label class="block text-sm font-semibold text-gray-400">Aux Reason Name</label>
    ...
    <script>
        function openCreateModal() {
            document.getElementById('modalTitle').innerText = 'Add New Data Aux Reason';
            document.getElementById('modalForm').action = "{{ route('data-aux-reason.store') }}";
        ...
        function openEditModal(id, name, status) {
            document.getElementById('modalTitle').innerText = 'Edit Data Aux Reason';
            document.getElementById('modalForm').action = "/data-aux-reason/" + id;
