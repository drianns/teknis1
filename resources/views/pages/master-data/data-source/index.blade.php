...
<div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
  <x-page-header title="Data Source" :breadcrumbs="['Master Data', 'Data Source']">
    <button onclick="openCreateModal()" class="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl transition-all shadow-lg shadow-blue-600/20 font-semibold text-sm">
      <i class='bx bx-plus-circle text-lg'></i><span>Add Source</span>
    </button>
  </x-page-header>
    <x-data-table>
      <x-slot name="thead">
...
<x-modal id="masterModal" title="Add Data Source" maxWidth="max-w-lg">
  ...
  <x-slot name="footer">
    ...
    <div id="master-data-config" class="hidden"
        data-ajax-url="{{ url('data-source/data') }}"
        data-store-url="{{ route('data-source.store') }}"
        data-update-base="{{ url('data-source') }}"
        data-delete-base="{{ url('data-source') }}"
        data-csrf="{{ csrf_token() }}"></div>

    @push('scripts')
        @vite('resources/js/pages/master-data/data-source.js')
    @endpush
@endsection
