...
<div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
  <x-page-header title="Data Activity" :breadcrumbs="['Master Data', 'Data Activity']">
    <button onclick="openCreateModal()" class="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl transition-all shadow-lg shadow-blue-600/20 font-semibold text-sm">
      <i class='bx bx-plus-circle text-lg'></i><span>Add Activity</span>
    </button>
  </x-page-header>
    <x-data-table>
      <x-slot name="thead">
...
<x-modal id="masterModal" title="Add Data Activity" maxWidth="max-w-lg">
  ...
  <x-slot name="footer">
    ...
    <div id="master-data-config" class="hidden"
        data-ajax-url="{{ url('data-activity/data') }}"
        data-store-url="{{ route('data-activity.store') }}"
        data-update-base="{{ url('data-activity') }}"
        data-delete-base="{{ url('data-activity') }}"
        data-csrf="{{ csrf_token() }}"></div>

    @push('scripts')
        @vite('resources/js/pages/master-data/data-activity.js')
    @endpush
</script>
@endsection
