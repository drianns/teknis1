...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <x-page-header title="Channel Ticket" :breadcrumbs="['Master Data', 'Channel Ticket']">
    ...
    <x-modal id="masterModal" title="Add Channel Ticket" maxWidth="max-w-lg">
    ...
    <div id="master-data-config" class="hidden"
        data-ajax-url="{{ url('channel-ticket/data') }}"
        data-store-url="{{ route('channel-ticket.store') }}"
        data-update-base="{{ url('channel-ticket') }}"
        data-delete-base="{{ url('channel-ticket') }}"
        data-csrf="{{ csrf_token() }}"></div>

    @push('scripts')
        @vite('resources/js/pages/master-data/channel-ticket.js')
    @endpush
@endsection
