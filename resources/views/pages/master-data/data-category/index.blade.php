...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <header class="flex-shrink-0 mb-3 px-6 pt-4">
            <div class="flex justify-between items-center mb-2">
                <h1 class="text-[28px] font-bold text-white tracking-tight">Data Category</h1>
            ...
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="text-blue-500 font-semibold">Data Category</span>
            </div>
        </header>
                ...
                <div class="table-wrapper flex-1 overflow-auto w-full custom-scrollbar">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody id="tableBody" class="divide-y divide-gray-700/50 text-sm text-gray-300">
                            <tr><td colspan="6" class="px-6 py-16 text-center text-gray-500">Loading...</td></tr>
                        </tbody>
                    </table>
                </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 py-12">
            ...
            <div class="relative bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-700 ring-1 ring-white/5">
                <div class="px-6 py-4 border-b border-gray-700 flex justify-between items-center bg-gray-900/50">
                    <h3 id="modalTitle" class="text-xl font-bold text-white">Add New Data Category</h3>
    ...
    <script>
        const AJAX_URL='{{ url("data-category/data") }}',STORE_URL='{{ route("data-category.store") }}',UPDATE_BASE='{{ url("data-category") }}',DELETE_BASE='{{ url("data-category") }}',CSRF='{{ csrf_token() }}';
        ...
        function renderTable(d){const b=document.getElementById('tableBody');if(!d.data||!d.data.length){b.innerHTML='<tr><td colspan="6" class="px-6 py-16 text-center"><div class="flex flex-col items-center text-gray-500"><i class=\'bx bx-data text-5xl mb-3 opacity-20\'></i><p>No data found</p></div></td></tr>';return;}b.innerHTML=d.data.map(i=>`<tr class="hover:bg-blue-500/[0.03] transition-colors"><td class="px-6 py-4 font-mono text-blue-400 font-medium text-center">${i.id}</td><td class="px-6 py-4 text-gray-300">${escHtml(i.brand_name)}</td><td class="px-6 py-4 text-gray-300">${escHtml(i.type_name)}</td><td class="px-6 py-4 text-gray-100 font-medium">${escHtml(i.name)}</td><td class="px-6 py-4 text-center"><span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold uppercase ${i.status==='Aktif'?'bg-emerald-500/10 text-emerald-400 ring-1 ring-emerald-500/20':'bg-rose-500/10 text-rose-400 ring-1 ring-rose-500/20'}">${i.status}</span></td><td class="px-6 py-4 text-center"><div class="flex justify-center gap-2"><button onclick="openEditModal(${i.id},${i.data_brand_name_id??'null'},${i.data_type_id??'null'},'${escJs(i.name)}','${escJs(i.status)}')" class="w-8 h-8 rounded-lg bg-blue-500/10 hover:bg-blue-500/20 flex items-center justify-center text-blue-400 border border-blue-500/20"><i class='bx bx-edit-alt'></i></button><button onclick="deleteData(${i.id})" class="w-8 h-8 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 flex items-center justify-center text-rose-400 border border-rose-500/20"><i class='bx bx-trash'></i></button></div></td></tr>`).join('');}
        ...
        function openCreateModal(){document.getElementById('modalTitle').innerText='Add New Data Category';document.getElementById('editId').value='';document.getElementById('modal_brand_id').value='';document.getElementById('modal_type_id').value='';document.getElementById('modal_name').value='';document.getElementById('modal_status').value='Aktif';document.getElementById('masterModal').classList.remove('hidden');}
        function openEditModal(id,brand_id,type_id,name,status){document.getElementById('modalTitle').innerText='Edit Data Category';document.getElementById('editId').value=id;document.getElementById('modal_brand_id').value=brand_id??'';document.getElementById('modal_type_id').value=type_id??'';document.getElementById('modal_name').value=name;document.getElementById('modal_status').value=status;document.getElementById('masterModal').classList.remove('hidden');}
        function closeModal(){document.getElementById('masterModal').classList.add('hidden');}
        function saveData(){const id=document.getElementById('editId').value,body=new URLSearchParams({_token:CSRF,data_brand_name_id:document.getElementById('modal_brand_id').value,data_type_id:document.getElementById('modal_type_id').value,name:document.getElementById('modal_name').value,status:document.getElementById('modal_status').value}),isEdit=id!=='';if(isEdit)body.append('_method','PUT');document.getElementById('saveBtn').disabled=true;document.getElementById('saveBtn').textContent='Saving...';fetch(isEdit?`${UPDATE_BASE}/${id}`:STORE_URL,{method:'POST',headers:{'X-CSRF-TOKEN':CSRF,'X-Requested-With':'XMLHttpRequest'},body}).then(r=>r.json()).then(res=>{if(res.success){closeModal();loadTable(currentPage);showToast(res.message,'success');}else showToast(Object.values(res.errors||{}).flat().join(', '),'error');}).catch(()=>showToast('Error.','error')).finally(()=>{document.getElementById('saveBtn').disabled=false;document.getElementById('saveBtn').textContent='Save Changes';});}
