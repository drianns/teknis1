...
<div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
  <x-page-header title="Department Escalation Unit" :breadcrumbs="['Master Data', 'Department Escalation Unit']">
...
<x-modal id="masterModal" title="Add Unit" maxWidth="max-w-lg">
  ...
  <x-slot name="footer">
    ...
    <div id="master-data-config" class="hidden"
        data-ajax-url="{{ url('department-escalation-unit/data') }}"
        data-store-url="{{ route('department-escalation-unit.store') }}"
        data-update-base="{{ url('department-escalation-unit') }}"
        data-delete-base="{{ url('department-escalation-unit') }}"
        data-csrf="{{ csrf_token() }}"></div>

    @push('scripts')
        @vite('resources/js/pages/master-data/department-escalation-unit.js')
    @endpush
@endsection
