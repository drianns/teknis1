...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <header class="flex-shrink-0 mb-3 px-6 pt-4">
            ...
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2 text-gray-600">/</span>
                <span class="current text-blue-500 font-semibold">Data Holiday</span>
            </div>
        </header>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-900/50">
                                ...
                                <th class="sticky top-0 z-10 bg-gray-900 px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Start Date</th>
                                <th class="sticky top-0 z-10 bg-gray-900 px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">End Date</th>
                        ...
                        <tbody class="divide-y divide-gray-700/50 text-sm text-gray-300">
                            ...
                            <tr class="hover:bg-blue-500/[0.03] transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-gray-400 font-mono text-xs">
                                    {{ $item->start_date ? \Carbon\Carbon::parse($item->start_date)->format('M d, Y h:i A') : '-' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-gray-400 font-mono text-xs">
                                    {{ $item->end_date ? \Carbon\Carbon::parse($item->end_date)->format('M d, Y h:i A') : '-' }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="relative flex justify-center" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.outside="open = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="absolute right-0 top-full mt-2 w-32 bg-gray-800 border border-gray-700 rounded-xl shadow-xl z-20 overflow-hidden ring-1 ring-white/5" style="display:none;">
                                            <button onclick="openEditModal({{ $item->id }}, '{{ addslashes($item->name) }}', '{{ $item->start_date }}', '{{ $item->end_date }}', '{{ $item->status }}')" class="w-full text-left px-4 py-2.5 text-sm text-gray-300 hover:bg-gray-700/50 hover:text-white flex items-center gap-3">
                                                <i class='bx bx-edit-alt text-blue-400 text-lg'></i>
                                                <span class="font-medium">Edit</span>
                                            </button>
                                            <form action="{{ route('data-holiday.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Silahkan konfirmasi untuk menghapus data ini?')">
                                                @csrf
                                                @method('DELETE')
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center text-gray-500">
                                        <i class='bx bx-calendar-event text-5xl mb-3 opacity-20'></i>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 py-12">
            ...
            <div class="relative bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-700 ring-1 ring-white/5">
                <div class="px-6 py-4 border-b border-gray-700 flex justify-between items-center bg-gray-900/50">
                    <h3 id="modalTitle" class="text-xl font-bold text-white tracking-tight">Add New Data Holiday</h3>
                ...
                <form id="modalForm" method="POST">
                    ...
                    <div class="p-6 space-y-5">
                        <div class="space-y-2">
                            <label class="block text-sm font-semibold text-gray-400">Holiday Name</label>
                            <input type="text" name="name" id="modal_name" class="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all placeholder-gray-600" placeholder="Enter name" required>
                        </div>

                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-400">Start Date</label>
                                <input type="datetime-local" name="start_date" id="modal_start_date" class="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all appearance-none" required>
                            </div>
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-400">End Date</label>
                                <input type="datetime-local" name="end_date" id="modal_end_date" class="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all appearance-none" required>
                            </div>
                        </div>
                    ...
                    <div class="px-6 py-4 border-t border-gray-700 bg-gray-900/50 flex justify-end gap-3">
                        ...
                        <button type="submit" class="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl shadow-lg shadow-blue-600/20 transition-all font-bold text-sm">Save Changes</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        function openCreateModal() {
            document.getElementById('modalTitle').innerText = 'Add New Data Holiday';
            document.getElementById('modalForm').action = "{{ route('data-holiday.store') }}";
        ...
        function openEditModal(id, name, start_date, end_date, status) {
            document.getElementById('modalTitle').innerText = 'Edit Data Holiday';
            document.getElementById('modalForm').action = "/data-holiday/" + id;
            document.getElementById('methodField').value = 'PUT';
            document.getElementById('modal_name').value = name;
            
            // Format format datetime-local (YYYY-MM-DDTHH:MM)
            if(start_date) {
                let s = new Date(start_date);
                document.getElementById('modal_start_date').value = s.toISOString().slice(0, 16);
            }
            if(end_date) {
                let e = new Date(end_date);
                document.getElementById('modal_end_date').value = e.toISOString().slice(0, 16);
            }
