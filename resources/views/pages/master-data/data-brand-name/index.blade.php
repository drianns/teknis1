...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <x-page-header title="Data Brand Name" :breadcrumbs="['Master Data', 'Data Brand Name']">
        ...
        <x-data-table>
            <x-slot name="thead">
                ...
                <th class="sticky top-0 z-10 bg-gray-900 px-6 py-4 w-40 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-center">Brand Category</th>
                <th class="sticky top-0 z-10 bg-gray-900 px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-left">Brand Name</th>
    ...
    <!-- Modal Create/Edit -->
    <x-modal id="masterModal" title="Add New Data Brand Name" maxWidth="max-w-lg">
    ...
    <div id="master-data-config" class="hidden"
        data-ajax-url="{{ url('data-brand-name/data') }}"
        data-store-url="{{ route('data-brand-name.store') }}"
        data-update-base="{{ url('data-brand-name') }}"
        data-delete-base="{{ url('data-brand-name') }}"
        data-csrf="{{ csrf_token() }}"></div>

    @push('scripts')
        @vite('resources/js/pages/master-data/data-brand-name.js')
    @endpush
@endsection
