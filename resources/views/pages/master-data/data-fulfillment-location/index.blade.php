...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <header class="flex-shrink-0 mb-3 px-6 pt-4">
            ...
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2 text-gray-600">/</span>
                <span class="current text-blue-500 font-semibold">Data Fulfillment Location</span>
            </div>
        </header>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-900/50">
                                ...
                                <th class="sticky top-0 z-10 bg-gray-900 px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Location Name</th>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="relative flex justify-center" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.outside="open = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="absolute right-0 top-full mt-2 w-32 bg-gray-800 border border-gray-700 rounded-xl shadow-xl z-20 overflow-hidden ring-1 ring-white/5" style="display:none;">
                                            ...
                                            <form action="{{ route('data-fulfillment-location.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Silahkan konfirmasi untuk menghapus data ini?')">
                                                @csrf
                                                @method('DELETE')
        ...
        <div class="flex items-center justify-center min-h-screen px-4 py-12">
            ...
            <div class="relative bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-700 ring-1 ring-white/5">
                <div class="px-6 py-4 border-b border-gray-700 flex justify-between items-center bg-gray-900/50">
                    <h3 id="modalTitle" class="text-xl font-bold text-white tracking-tight">Add New Data Fulfillment Location</h3>
    ...
    <script>
        function openCreateModal() {
            document.getElementById('modalTitle').innerText = 'Add New Data Fulfillment Location';
            document.getElementById('modalForm').action = "{{ route('data-fulfillment-location.store') }}";
        ...
        function openEditModal(id, name, status) {
            document.getElementById('modalTitle').innerText = 'Edit Data Fulfillment Location';
            document.getElementById('modalForm').action = "/data-fulfillment-location/" + id;
