...
<div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
  <x-page-header title="Data Max Handle" :breadcrumbs="['Master Data', 'Data Max Handle']">
    <button onclick="openCreateModal()" class="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl transition-all shadow-lg shadow-blue-600/20 font-semibold text-sm"><i class='bx bx-plus-circle text-lg'></i><span>Add Max Handle</span></button>
        ...
        <x-data-table>
            <x-slot name="thead">
                ...
                <th class="sticky top-0 z-10 bg-gray-900 px-6 py-4 w-32 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-center">Max Handle</th>
...
<x-modal id="masterModal" title="Add Max Handle" maxWidth="max-w-lg">
  ...
  <x-slot name="footer">
    ...
    <div id="master-data-config" class="hidden"
        data-ajax-url="{{ url('data-max-handle/data') }}"
        data-store-url="{{ route('data-max-handle.store') }}"
        data-update-base="{{ url('data-max-handle') }}"
        data-delete-base="{{ url('data-max-handle') }}"
        data-csrf="{{ csrf_token() }}"></div>

    @push('scripts')
        @vite('resources/js/pages/master-data/data-max-handle.js')
    @endpush
@endsection
