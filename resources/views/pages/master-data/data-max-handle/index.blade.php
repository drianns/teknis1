...
<div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
  <header class="flex-shrink-0 mb-3 px-6 pt-4">
    <div class="flex justify-between items-center mb-2">
      ...
      <button onclick="openCreateModal()" class="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl transition-all shadow-lg shadow-blue-600/20 font-semibold text-sm"><i class='bx bx-plus-circle text-lg'></i><span>Add Max Handle</span></button>
    </div>
    <div class="flex items-center gap-2 text-sm text-gray-400">
      <span>Home</span><span class="mx-2 text-gray-600">/</span><span>Master Data</span><span class="mx-2 text-gray-600">/</span><span class="text-blue-500 font-semibold">Data Max Handle</span>
    </div>
  </header>
  ...
  <div class="flex items-center justify-center min-h-screen px-4 py-12">
    ...
    <div class="relative bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-700">
      <div class="px-6 py-4 border-b border-gray-700 flex justify-between items-center bg-gray-900/50">
        <h3 id="modalTitle" class="text-xl font-bold text-white">Add Max Handle</h3>
...
<script>
const AJAX_URL='{{ url("data-max-handle/data") }}',STORE_URL='{{ route("data-max-handle.store") }}',UPDATE_BASE='{{ url("data-max-handle") }}',DELETE_BASE='{{ url("data-max-handle") }}',CSRF='{{ csrf_token() }}';
...
function openCreateModal(){document.getElementById('modalTitle').innerText='Add Data Max Handle';document.getElementById('editId').value='';document.getElementById('modal_name').value='';document.getElementById('modal_status').value='Aktif';document.getElementById('masterModal').classList.remove('hidden');}
function openEditModal(id,name,status){document.getElementById('modalTitle').innerText='Edit Data Max Handle';document.getElementById('editId').value=id;document.getElementById('modal_name').value=name;document.getElementById('modal_status').value=status;document.getElementById('masterModal').classList.remove('hidden');}
