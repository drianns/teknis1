...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <header class="flex-shrink-0 mb-3 px-6 pt-4">
            <div class="flex justify-between items-center mb-2">
                <h1 class="text-[28px] font-bold text-white tracking-tight">Data Site</h1>
                ...
                <div class="table-wrapper flex-1 overflow-auto w-full custom-scrollbar">
                    <table class="w-full text-left border-collapse">
                        <thead><tr class="bg-gray-900/50">
                            ...
                            <th class="sticky top-0 z-10 bg-gray-900 px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Site Name</th>
                            <th class="sticky top-0 z-10 bg-gray-900 px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Location</th>
                        ...
                        <tbody id="tableBody" class="divide-y divide-gray-700/50 text-sm text-gray-300"><tr><td colspan="5" class="px-6 py-16 text-center text-gray-500">Loading...</td></tr></tbody>
                    </table>
                </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 py-12">
            ...
            <div class="relative bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-700 ring-1 ring-white/5">
                <div class="px-6 py-4 border-b border-gray-700 flex justify-between items-center bg-gray-900/50">
                    <h3 id="modalTitle" class="text-xl font-bold text-white tracking-tight">Add New Data Site</h3>
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div class="space-y-2"><label class="block text-sm font-semibold text-gray-400">Site Name</label>
                    ...
                    <div class="space-y-2"><label class="block text-sm font-semibold text-gray-400">Location</label>
                        <input type="text" id="modal_location" class="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 placeholder-gray-600" placeholder="Enter location">
                    </div>
    ...
    <script>
        const AJAX_URL='{{ url("data-site/data") }}',STORE_URL='{{ route("data-site.store") }}',UPDATE_BASE='{{ url("data-site") }}',DELETE_BASE='{{ url("data-site") }}',CSRF='{{ csrf_token() }}';
        let currentPage=1,searchTimer=null;
        function loadTable(page=1){currentPage=page;const s=document.getElementById('searchInput').value,p=document.getElementById('perPage').value;fetch(`${AJAX_URL}?page=${page}&search=${encodeURIComponent(s)}&per_page=${p}`,{headers:{'X-Requested-With':'XMLHttpRequest'}}).then(r=>r.json()).then(d=>{renderTable(d);renderPagination(d);}).catch(()=>{document.getElementById('tableBody').innerHTML='<tr><td colspan="5" class="px-6 py-16 text-center text-red-400">Failed to load data.</td></tr>';});}
        function renderTable(d){const b=document.getElementById('tableBody');if(!d.data||!d.data.length){b.innerHTML='<tr><td colspan="5" class="px-6 py-16 text-center text-gray-500">No data found</td></tr>';return;}b.innerHTML=d.data.map(i=>`<tr class="hover:bg-blue-500/[0.03] transition-colors"><td class="px-6 py-4 font-mono text-blue-400 font-medium text-center">${i.id}</td><td class="px-6 py-4 text-gray-100 font-medium">${escHtml(i.name)}</td><td class="px-6 py-4 text-gray-400">${escHtml(i.location)}</td><td class="px-6 py-4 text-center"><span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold uppercase ${i.status==='Aktif'?'bg-emerald-500/10 text-emerald-400 ring-1 ring-emerald-500/20':'bg-rose-500/10 text-rose-400 ring-1 ring-rose-500/20'}">${i.status}</span></td><td class="px-6 py-4 text-center"><div class="flex justify-center gap-2"><button onclick="openEditModal(${i.id},'${escJs(i.name)}','${escJs(i.location)}','${escJs(i.status)}')" class="w-8 h-8 rounded-lg bg-blue-500/10 hover:bg-blue-500/20 flex items-center justify-center text-blue-400 border border-blue-500/20"><i class='bx bx-edit-alt'></i></button><button onclick="deleteData(${i.id})" class="w-8 h-8 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 flex items-center justify-center text-rose-400 border border-rose-500/20"><i class='bx bx-trash'></i></button></div></td></tr>`).join('');}
