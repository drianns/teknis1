...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <x-page-header title="Data Site" :breadcrumbs="['Master Data', 'Data Site']">
    ...
    <x-modal id="masterModal" title="Add New Data Site" maxWidth="max-w-lg">
    ...
    <div id="master-data-config" class="hidden"
        data-ajax-url="{{ url('data-site/data') }}"
        data-store-url="{{ route('data-site.store') }}"
        data-update-base="{{ url('data-site') }}"
        data-delete-base="{{ url('data-site') }}"
        data-csrf="{{ csrf_token() }}"></div>

    @push('scripts')
        @vite('resources/js/pages/master-data/data-site.js')
    @endpush
    </script>
@endsection
