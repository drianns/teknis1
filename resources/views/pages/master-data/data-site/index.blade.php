...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <header class="flex-shrink-0 mb-3 px-6 pt-4">
            ...
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2 text-gray-600">/</span>
                <span class="current text-blue-500 font-semibold">Data Site</span>
            </div>
        </header>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-700/50 text-sm text-gray-300">
                            ...
                            <tr class="hover:bg-blue-500/[0.03] transition-colors">
                                ...
                                <td class="px-6 py-4 text-gray-400">{{ $item->location }}</td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="relative flex justify-center" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.outside="open = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="absolute right-0 top-full mt-2 w-32 bg-gray-800 border border-gray-700 rounded-xl shadow-xl z-20 overflow-hidden ring-1 ring-white/5" style="display:none;">
                                            <button onclick="openEditModal({{ $item->id }}, '{{ addslashes($item->name) }}', '{{ addslashes($item->location) }}', '{{ $item->status }}')" class="w-full text-left px-4 py-2.5 text-sm text-gray-300 hover:bg-gray-700/50 hover:text-white flex items-center gap-3">
                                                <i class='bx bx-edit-alt text-blue-400 text-lg'></i>
                                                <span class="font-medium">Edit</span>
                                            </button>
                                            <form action="{{ route('data-site.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Silahkan konfirmasi untuk menghapus data ini?')">
                                                @csrf
                                                @method('DELETE')
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center text-gray-500">
                                        <i class='bx bx-map-pin text-5xl mb-3 opacity-20'></i>
            ...
            <div class="relative bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-700 ring-1 ring-white/5">
                ...
                <form id="modalForm" method="POST">
                    ...
                    <div class="p-6 space-y-5">
                        <div class="space-y-2">
                            <label class="block text-sm font-semibold text-gray-400">Site Name</label>
                            <input type="text" name="name" id="modal_name" class="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all placeholder-gray-600" placeholder="Enter site name" required>
                        </div>
                        
                        <div class="space-y-2">
                            <label class="block text-sm font-semibold text-gray-400">Location</label>
                            <input type="text" name="location" id="modal_location" class="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all placeholder-gray-600" placeholder="Enter location" required>
                        </div>
    ...
    <script>
        function openCreateModal() {
            document.getElementById('modalTitle').innerText = 'Add New Data Site';
            document.getElementById('modalForm').action = "{{ route('data-site.store') }}";
        ...
        function openEditModal(id, name, location, status) {
            document.getElementById('modalTitle').innerText = 'Edit Data Site';
            document.getElementById('modalForm').action = "/data-site/" + id;
