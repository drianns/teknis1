...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <header class="flex-shrink-0 mb-3 px-6 pt-4">
            ...
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2 text-gray-600">/</span>
                <span class="current text-blue-500 font-semibold">Data Meta</span>
            </div>
        </header>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-900/50">
                                ...
                                <th class="sticky top-0 z-10 bg-gray-900 px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Created By</th>
                            ...
                            <tr class="hover:bg-blue-500/[0.03] transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-gray-100 font-medium">{{ $item->brand->name ?? '-' }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-gray-400 whitespace-nowrap">{{ $item->dataType->name ?? '-' }}</td>
                                <td class="px-6 py-4 text-gray-400 whitespace-nowrap">{{ $item->category->name ?? '-' }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex flex-col">
                                        <span class="text-gray-300 font-medium">{{ $item->created_by ?? 'Admin' }}</span>
                                        <span class="text-[10px] text-gray-500">{{ $item->created_at->format('M d, Y h:i A') }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="relative flex justify-center" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.outside="open = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="absolute right-0 top-full mt-2 w-32 bg-gray-800 border border-gray-700 rounded-xl shadow-xl z-20 overflow-hidden ring-1 ring-white/5" style="display:none;">
                                            <button onclick="openEditModal({{ $item->id }}, {{ $item->data_brand_name_id ?? 'null' }}, {{ $item->data_type_id ?? 'null' }}, {{ $item->data_category_id ?? 'null' }}, '{{ addslashes($item->name) }}', '{{ $item->status }}')" class="w-full text-left px-4 py-2.5 text-sm text-gray-300 hover:bg-gray-700/50 hover:text-white flex items-center gap-3">
                                                <i class='bx bx-edit-alt text-blue-400 text-lg'></i>
                                                <span class="font-medium">Edit</span>
                                            </button>
                                            <form action="{{ route('data-meta.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Silahkan konfirmasi untuk menghapus data ini?')">
                                                @csrf
                                                @method('DELETE')
        ...
        <div class="flex items-center justify-center min-h-screen px-4 py-12">
            ...
            <div class="relative bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-700 ring-1 ring-white/5">
                <div class="px-6 py-4 border-b border-gray-700 flex justify-between items-center bg-gray-900/50">
                    <h3 id="modalTitle" class="text-xl font-bold text-white tracking-tight">Add New Data Meta</h3>
                ...
                <form id="modalForm" method="POST">
                    ...
                    <div class="p-6 space-y-5">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-400">Brand Name</label>
                            ...
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-400">Data Type</label>
                        ...
                        <div class="space-y-2">
                            <label class="block text-sm font-semibold text-gray-400">Meta Name</label>
                            <input type="text" name="name" id="modal_name" class="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all placeholder-gray-600" placeholder="Enter meta name" required>
                        </div>
    ...
    <script>
        function openCreateModal() {
            document.getElementById('modalTitle').innerText = 'Add New Data Meta';
            document.getElementById('modalForm').action = "{{ route('data-meta.store') }}";
        ...
        function openEditModal(id, brand_id, type_id, category_id, name, status) {
            document.getElementById('modalTitle').innerText = 'Edit Data Meta';
            document.getElementById('modalForm').action = "/data-meta/" + id;
