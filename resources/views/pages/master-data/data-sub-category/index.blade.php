...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <header class="flex-shrink-0 mb-3 px-6 pt-4">
            ...
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2 text-gray-600">/</span>
                <span class="current text-blue-500 font-semibold">Data Sub Category</span>
            </div>
        </header>
                    ...
                    <table class="w-full text-left border-collapse min-w-[1200px]">
                        <thead>
                            <tr class="bg-gray-900/50">
                                ...
                                <th class="sticky top-0 z-10 bg-gray-900 px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest font-bold text-white">Sub Category</th>
                                <th class="sticky top-0 z-10 bg-gray-900 px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Unit / Layer / SLA</th>
                        ...
                        <tbody class="divide-y divide-gray-700/50 text-sm text-gray-300">
                            ...
                            <tr class="hover:bg-blue-500/[0.03] transition-colors text-[13px]">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-gray-400">{{ $item->dataType->name ?? '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-gray-400">{{ $item->category->name ?? '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-gray-400">{{ $item->meta->name ?? '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        <div class="w-1.5 h-1.5 rounded-full bg-blue-500"></div>
                                        <span class="text-gray-100 font-semibold">{{ $item->name }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex flex-col gap-0.5">
                                        <span class="text-gray-300 text-[12px] font-medium">{{ $item->escalationUnit->name ?? '-' }}</span>
                                        <div class="flex items-center gap-2 text-[10px] text-gray-500">
                                            <span>Layer: {{ $item->escalation_layer ?? '-' }}</span>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="relative flex justify-center" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.outside="open = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="absolute right-0 top-full mt-2 w-32 bg-gray-800 border border-gray-700 rounded-xl shadow-xl z-20 overflow-hidden ring-1 ring-white/5" style="display:none;">
                                            <button onclick="openEditModal({{ $item->id }}, {{ $item->data_brand_name_id ?? 'null' }}, {{ $item->data_type_id ?? 'null' }}, {{ $item->data_category_id ?? 'null' }}, {{ $item->data_meta_id ?? 'null' }}, '{{ addslashes($item->name) }}', {{ $item->department_escalation_unit_id ?? 'null' }}, {{ $item->escalation_layer ?? 'null' }}, {{ $item->sla ?? 'null' }}, '{{ $item->status }}')" class="w-full text-left px-4 py-2.5 text-sm text-gray-300 hover:bg-gray-700/50 hover:text-white flex items-center gap-3">
                                                <i class='bx bx-edit-alt text-blue-400 text-lg'></i>
                                                <span class="font-medium">Edit</span>
                                            </button>
                                            <form action="{{ route('data-sub-category.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Silahkan konfirmasi untuk menghapus data ini?')">
                                                @csrf
                                                @method('DELETE')
                            ...
                            <tr>
                                <td colspan="9" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center text-gray-500">
                                        ...
                                        <p>No sub category records found</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
        ...
        <div class="flex items-center justify-center min-h-screen px-4 py-12">
            ...
            <div class="relative bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden border border-gray-700 ring-1 ring-white/5">
                <div class="px-6 py-4 border-b border-gray-700 flex justify-between items-center bg-gray-900/50">
                    <h3 id="modalTitle" class="text-xl font-bold text-white tracking-tight text-glow-blue">Add New Data Sub Category</h3>
                    ...
                    <div class="p-6 space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div class="space-y-2">
                                ...
                                <select name="data_type_id" id="modal_type_id" class="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all appearance-none" required>
                                    <option value="">Select Type</option>
                                    @foreach($types as $type)
                                        <option value="{{ $type->id }}">{{ $type->name }} ({{ $type->brand->name ?? '-' }})</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-400">Category</label>
                                <select name="data_category_id" id="modal_category_id" class="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all appearance-none" required>
                                    <option value="">Select Category</option>
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}">{{ $category->name }} ({{ $category->dataType->name ?? '-' }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-400">Meta</label>
                                <select name="data_meta_id" id="modal_meta_id" class="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all appearance-none" required>
                                    <option value="">Select Meta</option>
                                    @foreach($metas as $meta)
                                        <option value="{{ $meta->id }}">{{ $meta->name }} ({{ $meta->category->name ?? '-' }})</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="name" id="modal_name" class="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all placeholder-gray-600" placeholder="Enter sub category name" required>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                            <div class="space-y-2 col-span-1">
                                <label class="block text-sm font-semibold text-gray-400">Escalation Unit</label>
                                <select name="department_escalation_unit_id" id="modal_unit_id" class="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all appearance-none">
                            ...
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-400">Layer</label>
                                <input type="number" name="escalation_layer" id="modal_layer" class="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all placeholder-gray-600" placeholder="Layer">
                            </div>
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-400">SLA (Hours)</label>
                                <input type="number" name="sla" id="modal_sla" class="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all placeholder-gray-600" placeholder="SLA">
                            </div>
                        </div>
    ...
    <script>
        function openCreateModal() {
            ...
            document.getElementById('modalForm').action = "{{ route('data-sub-category.store') }}";
        ...
        function openEditModal(id, brand_id, type_id, category_id, meta_id, name, unit_id, layer, sla, status) {
            document.getElementById('modalTitle').innerText = 'Edit Data Sub Category';
            document.getElementById('modalForm').action = "/data-sub-category/" + id;
            document.getElementById('methodField').value = 'PUT';
            document.getElementById('modal_brand_id').value = brand_id;
            document.getElementById('modal_type_id').value = type_id;
            document.getElementById('modal_category_id').value = category_id;
            document.getElementById('modal_meta_id').value = meta_id;
            document.getElementById('modal_name').value = name;
            document.getElementById('modal_unit_id').value = unit_id;
