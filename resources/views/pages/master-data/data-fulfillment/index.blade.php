...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <x-page-header title="Data Fulfillment" :breadcrumbs="['Master Data', 'Data Fulfillment']">
        ...
        <x-data-table>
            <x-slot name="thead">
                ...
                <th class="sticky top-0 z-10 bg-gray-900 px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-left">Customer Status</th>
    ...
    <x-modal id="masterModal" title="Add New Data Fulfillment" maxWidth="max-w-lg">
    ...
    <div id="master-data-config" class="hidden"
        data-ajax-url="{{ url('data-fulfillment/data') }}"
        data-store-url="{{ route('data-fulfillment.store') }}"
        data-update-base="{{ url('data-fulfillment') }}"
        data-delete-base="{{ url('data-fulfillment') }}"
        data-csrf="{{ csrf_token() }}"></div>

    @push('scripts')
        @vite('resources/js/pages/master-data/data-fulfillment.js')
    @endpush
    </script>
@endsection
