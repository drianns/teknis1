                ...
                <div class="table-wrapper flex-1 overflow-auto w-full custom-scrollbar">
                    <table class="w-full text-left border-collapse" style="min-width:3200px">
                        ...
                        <tbody id="table-body" class="divide-y divide-gray-700/50 text-sm text-gray-300">
                            <tr>
                                <td colspan="22" class="px-4 py-16 text-center text-gray-500">
...
<script>
    ...
    function loadTable(page = 1) {
        ...
        .catch(err => {
            console.error(err);
            tableBody.innerHTML = '<tr><td colspan="22" class="px-4 py-12 text-center text-red-400 font-bold italic">Error loading data. Please try again.</td></tr>';
        });
    }
    ...
    function renderTable(rows) {
        ...
        if (!rows || rows.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="22" class="px-4 py-16 text-center text-gray-500"><div class="flex flex-col items-center justify-center gap-2"><i class="bx bx-data text-5xl mb-3 opacity-20"></i><p class="text-sm font-medium">No EPIC configurations found</p></div></td></tr>';
            return;
        }
        ...
        tableBody.innerHTML = rows.map(row => `
            <tr class="hover:bg-blue-500/[0.03] transition-colors group/row">
                ...
                <td class="px-4 py-3 text-blue-300 font-semibold text-xs whitespace-nowrap">${esc(row.aes)}</td>
                <td class="px-4 py-3 text-gray-300 whitespace-nowrap">${esc(row.aes_user)}</td>
                <td class="px-4 py-3 text-gray-400 font-mono whitespace-nowrap">${row.aes_pass ? '••••••••' : '-'}</td>
                <td class="px-4 py-3 text-gray-300 text-center whitespace-nowrap">${esc(row.port)}</td>
                <td class="px-4 py-3 text-gray-300 whitespace-nowrap">${esc(row.ip_db)}</td>
                <td class="px-4 py-3 text-gray-300 whitespace-nowrap">${esc(row.db_user)}</td>
                <td class="px-4 py-3 text-gray-400 font-mono whitespace-nowrap">${row.db_pass ? '••••••••' : '-'}</td>
                <td class="px-4 py-3 text-gray-300 whitespace-nowrap">${esc(row.db_name)}</td>
                <td class="px-4 py-3 text-gray-300 text-center whitespace-nowrap">${esc(row.dial_code) || '—'}</td>
                <td class="px-4 py-3 text-gray-500 italic whitespace-nowrap">${esc(row.call_history) || '—'}</td>
                <td class="px-4 py-3 text-blue-400 text-xs whitespace-nowrap">${esc(row.agent_ep)}</td>
                <td class="px-4 py-3 text-blue-400 text-xs whitespace-nowrap">${esc(row.inbound_ep)}</td>
                <td class="px-4 py-3 text-gray-500 italic whitespace-nowrap">${esc(row.outbound_ep) || '—'}</td>
                <td class="px-4 py-3 text-gray-400 text-xs font-mono whitespace-nowrap">${esc(row.browser_path)}</td>
                <td class="px-4 py-3 text-gray-300 text-center whitespace-nowrap">${esc(row.theme)}</td>
                <td class="px-4 py-3 text-gray-300 text-center whitespace-nowrap">${esc(row.acw)}</td>
                <td class="px-4 py-3 text-gray-300 font-mono text-center whitespace-nowrap">${esc(row.pbx_login)}</td>
                <td class="px-4 py-3 text-gray-300 font-mono text-center whitespace-nowrap">${esc(row.pbx_logout)}</td>
                <td class="px-4 py-3 text-gray-300 font-mono text-center whitespace-nowrap">${esc(row.pbx_aux)}</td>
                <td class="px-4 py-3 text-gray-300 font-mono text-center whitespace-nowrap">${esc(row.pbx_autoin)}</td>
