...
@section('content')
        ...
        <div class="flex-1 flex flex-col p-4 lg:p-6 lg:pt-0 pt-0 overflow-hidden w-full">
            <div class="table-section bg-gray-800/80 backdrop-blur-md rounded-2xl border border-gray-700/50 shadow-2xl overflow-hidden ring-1 ring-white/5 flex-1 flex flex-col min-h-0">
                <div class="table-controls px-4 py-3 border-b border-gray-700/50 bg-gray-800/30 flex flex-wrap justify-between items-center gap-4">
                    ...
                    <div class="relative">
                        <input type="text" id="search-input" oninput="debounceSearch()" class="bg-gray-800 border border-gray-700 rounded-xl pl-10 pr-4 py-2 text-sm text-gray-300 focus:outline-none focus:border-blue-500 w-64 placeholder-gray-500 shadow-inner" placeholder="Search Agent Name or Email..." />
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead class="bg-gray-900/50">
                            <tr>
                                ...
                                <th class="sticky top-0 z-10 bg-gray-900 px-4 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Nama Agent</th>
    ...
    @push('scripts')
<script>
        const AJAX_URL = "{{ route('setup-channel-email.setting-agent.getData') }}";
        let searchTimeout;
        ...
        function loadTable(page = 1) {
            ...
            .catch(err => {
                console.error(err);
                tableBody.innerHTML = '<tr><td colspan="5" class="px-4 py-12 text-center text-red-400 font-bold italic">Error loading data.</td></tr>';
            });
        }
        ...
        function renderTable(rows) {
            ...
            if (!rows || rows.length === 0) {
                tableBody.innerHTML = '<tr><td colspan="5" class="px-4 py-12 text-center text-gray-500"><div class="flex flex-col items-center gap-2"><i class="bx bx-folder-open text-4xl opacity-20"></i><p>No agent email settings found</p></div></td></tr>';
                return;
            }
            ...
            tableBody.innerHTML = rows.map(row => {
                ...
                <tr class="hover:bg-blue-500/[0.03] transition-colors group/row">
                    ...
                    <td class="px-4 py-3 font-medium text-white">${esc(row.username || user.name || '-')}</td>
                    <td class="px-4 py-3 text-gray-400">${esc(user.email || '-')}</td>
                    <td class="px-4 py-3 text-center">
                        <span class="bg-emerald-500/20 text-emerald-400 px-3 py-1 rounded-full text-[10px] font-bold border border-emerald-500/30 whitespace-nowrap uppercase tracking-wider">
                            Aktif
                        </span>
                    </td>
                    <td class="px-4 py-3 text-center">
