...
<script>
    ...
    function renderTable(rows) {
        ...
        if (!rows || rows.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="4" class="px-4 py-12 text-center text-gray-500"><div class="flex flex-col items-center gap-2"><i class="bx bx-folder-open text-4xl opacity-20"></i><p>No filter data found</p></div></td></tr>';
            return;
        }
        ...
        tableBody.innerHTML = rows.map(row => `
            <tr class="hover:bg-blue-500/[0.03] transition-colors group/row">
                ...
                <td class="px-4 py-3 font-medium text-white">${esc(row.days)} Hari</td>
                <td class="px-4 py-3 text-gray-300 font-normal">${esc(row.description)}</td>
