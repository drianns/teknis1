...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-900">
        <header class="flex-shrink-0 mb-3 px-6 pt-4">
            <h1 class="text-[28px] font-bold text-white tracking-tight mb-2">Data Jumlah Filter</h1>
            <div class="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span class="mx-2">/</span>
                <span class="current text-blue-500 font-semibold">Data Jumlah Filter</span>
            </div>
        </header>
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-900/50">
                        <tr>
                            ...
                            <th class="sticky top-0 z-10 bg-gray-900 px-4 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Jumlah Filter</th>
    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-gray-800 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-gray-700">
            <div class="bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4 border-b border-gray-700 flex justify-between items-center">
                <h3 class="text-xl font-bold text-white">Edit Data Jumlah Filter</h3>
            ...
            <form id="filterForm" onsubmit="submitForm(event)">
                ...
                <div class="bg-gray-800 px-4 py-5 sm:p-6 space-y-5">
                    <div>
                        <label class="block text-sm font-medium text-gray-400 mb-2">Jumlah Hari <span class="text-red-500">*</span></label>
                        <input type="number" id="days" name="days" min="1" class="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-2.5 text-gray-300 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" required>
                        <p class="mt-2 text-xs text-gray-500">Masukkan jumlah hari untuk difilter.</p>
                    </div>
                </div>
...
<script>
    ...
    const UPDATE_URL_TEMPLATE = "{{ url('setup-channel-email/filter-jumlah-hari') }}";
    let searchTimeout;
    ...
    function loadTable(page = 1) {
        ...
        .catch(err => {
            console.error(err);
            tableBody.innerHTML = '<tr><td colspan="3" class="px-4 py-12 text-center text-red-400 font-bold italic">Error loading data. Please try again.</td></tr>';
        });
    }
    ...
    function renderTable(rows) {
        ...
        if (!rows || rows.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="3" class="px-4 py-12 text-center text-gray-500"><div class="flex flex-col items-center gap-2"><i class="bx bx-folder-open text-4xl opacity-20"></i><p>No filter data found</p></div></td></tr>';
            return;
        }
        ...
        tableBody.innerHTML = rows.map(row => {
            ...
            <tr class="hover:bg-blue-500/[0.03] transition-colors group/row border-b border-gray-700/30">
                ...
                <td class="px-4 py-4 font-medium text-white">${row.days}</td>
    ...
    function editModal(rowStr) {
        try {
            ...
            document.getElementById('filter_id').value = row.id;
            document.getElementById('days').value = row.days;
